...
void AssembleModule( const int n_node, int& n_dof, int& total_dof, int& n_pre_disp, int& n_load, int& nnode_ele, int& n_elem, 
		  ...
		  arma::mat& Kfp, arma::colvec& Pf,  arma::colvec& Pp, arma::colvec& Uf, arma::colvec& Up,  arma::colvec& Uur, 
		  arma::colvec& Pur, arma::colvec& coords, arma::colvec& h, arma::icolvec& force_node, arma::colvec& force_val,  arma::colvec& elem_area)
{ 
...
//Assemble Module:
  ...
  for(int iel=0; iel<n_elem; iel++){
  	p1 = elem_load(iel)/h(iel) * (coords(iel+1) * h(iel) - 0.5 * (coords(iel+1) * coords(iel+1)) + 0.5 * (coords(iel) * coords(iel))) * elem_area(iel);
  	p2 = elem_load(iel)/h(iel) * (0.5 * (coords(iel+1) * coords(iel+1)) - (0.5 * coords(iel) * coords(iel)) - coords(iel) * h(iel)) * elem_area(iel);
  	Pel << p1 << arma::endr
  		<< p2 << arma::endr;
  ...
  for(int iel=0; iel<n_elem; iel++){
  	double k;
  	k = elem_stiff(iel) * elem_area(iel) / h(iel);	//ith elem stiffness 
  	Kel << k << -k << arma::endr
  		<< -k << k << arma::endr;
