...
/*
  ...
  if (fout.is_open() ) {
	  ...
	  fout >> n_node >> n_elem >> n_load >> n_pre_disp >> n_trac >> n_mat >> n_bload >> Model2D;
	  //cout<<n_node<<"\t"<<n_elem<<"\t"<<n_load<<"\t"<<n_pre_disp<<"\t"<<"\t" << n_trac <<"\t"<< n_mat <<"\t"<< n_bload<<"\t"<<Model2D<<"\n";
  ...
  trac_ele  = arma::zeros<arma::imat>(n_trac,2);//% first entry is the element, second entry is the local edge number
  trac_val  = arma::zeros<arma::mat>(n_trac,3);//% first entry is the magnitude, the other two entries correspond to the direction unit vector (ex, ey)

  arma::mat material = arma::zeros<arma::mat>(n_mat,3); 
  arma::mat bodyload = arma::zeros<arma::mat>(n_bload,2);
  ...
  for (int i=0;i<n_bload;i++ ) {
	  fout>>temp>> bodyload(i,0) >> bodyload(i,1);
   }
  //cout<<bodyload;
  ...
  for (int i=0;i<n_elem;i++ ) {
	  ...
	  elem_load(i,1) = bodyload(load_index-1,1);	  
  ...
  for (int i=0;i<n_trac;i++ ) {
    fout >> trac_ele(i,0) >> trac_ele(i,1) >> trac_val(i,0) >> trac_val(i,1) >> trac_val(i,2);
  }
