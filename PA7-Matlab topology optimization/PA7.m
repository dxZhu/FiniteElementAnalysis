...
function PA7(nelx,nely,volfrac,penal,rmin);
% INITIALIZE
x(1:nely,1:nelx) = volfrac; 
% ADD A HOLE 
for ely = 1:nely
  for elx = 1:nelx
    if sqrt((elx-nelx*2/3.)^2+(ely-nely/2.)^2)<nely/3.
        passive(ely,elx) = 1;
        x(ely,elx) = 0.001;
    else
        passive(ely,elx) = 0;
...
% DEFINE LOAD 1
for elx = (nelx/2+1):(nelx+1)
    node = (nely+1)*(elx-1)+1;
    F(2*node,1) = 10;
end
F(2*((nely+1)*nelx/2+1),1) = 5;
F(2*((nely+1)*nelx+1),1) = 5;
% DEFINE LOAD 2
for elx = (nelx/2+1):(nelx+1)
    node = (nely+1)*(elx-1)+nely+1;
    F(2*node,2) = -10;
end
F(2*((nely+1)*nelx/2+nely+1),2) = -5;
F(2*((nely+1)*nelx+nely+1),2) = -5;
